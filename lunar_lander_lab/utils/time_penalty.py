"""Reward-shaping and action-constraint wrappers, and the sweeps that use them.

The evaluation primitives these sweeps score against (`evaluate_controller_natural`,
engine/fuel accounting, the success threshold) now live in `evaluation.py` --
they are shared by every study in the repo and did not belong in a module
named after one experiment.
"""
...
class EngineLockoutWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        if invert and lockout_steps is not None:
            # "Block after N steps" is a different mechanism with no
            # motivating hypothesis behind it -- refuse rather than silently
            # do something nobody asked for.
            raise ValueError("invert applies to altitude_threshold, not lockout_steps")
    ...
    def _gate_active(self) -> bool:
        ...
        if self.invert:
            # Terminal cutoff: block the engine for the last stretch, the
            # mirror of the initial lockout (Arc 3 Phase D).
